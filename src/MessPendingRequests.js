...
import './MessPendingRequests.css';

const MessPendingRequests = () => {
  ...
  useEffect(() => {
    if (department) {
      axios
        .get(`http://localhost:3001/api/mess-pending-requests?department=${department}`)
        .then((response) => {
  ...
  const handleApprove = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/mess-approved-requests", {
  ...
  const handlePause = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/mess-paused-requests", {
        requestIds: selectedRequestIds,
      });
  ...
  return (
    <div className="mess-pending-requests">
...
export default MessPendingRequests;
