...
import './HostelPendingRequests.css';

const HostelPendingRequests = () => {
  ...
  useEffect(() => {
    if (department) {
      axios
        .get(`http://localhost:3001/api/hostel-pending-requests?department=${department}`)
        .then((response) => {
  ...
  const handleApprove = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/hostel-approved-requests", {
  ...
  const handlePause = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/hostel-paused-requests", {
        requestIds: selectedRequestIds,
      });
  ...
  return (
    <div className="hostel-pending-requests">
...
export default HostelPendingRequests;
