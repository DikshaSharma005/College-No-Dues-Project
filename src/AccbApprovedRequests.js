...
import './AccbApprovedRequests.css';

const AccbApprovedRequests = () => {
  ...
  useEffect(() => {
    if (department) {
    axios
    .get(`http://localhost:3001/api/accb-approved-requests?department=${department}`)
      .then((response) => {
...
export default AccbApprovedRequests;
