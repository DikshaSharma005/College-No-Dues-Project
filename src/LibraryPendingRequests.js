...
import './LibraryPendingRequests.css';

const LibraryPendingRequests = () => {
  ...
  useEffect(() => {
    if (department) {
      axios
        .get(`http://localhost:3001/api/library-pending-requests?department=${department}`)
        .then((response) => {
  ...
  const handleApprove = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/library-approved-requests", {
  ...
  const handlePause = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/library-paused-requests", {
        requestIds: selectedRequestIds,
      });
  ...
  return (
    <div className="library-pending-requests">
...
export default LibraryPendingRequests;
