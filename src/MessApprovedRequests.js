...
import './MessApprovedRequests.css';

const MessApprovedRequests = () => {
  ...
  useEffect(() => {
    if (department) {
    axios
    .get(`http://localhost:3001/api/mess-approved-requests?department=${department}`)
      .then((response) => {
...
export default MessApprovedRequests;
