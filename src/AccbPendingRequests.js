...
import './AccbPendingRequests.css';

const AccbPendingRequests = () => {
  ...
  useEffect(() => {
    if (department) {
      axios
        .get(`http://localhost:3001/api/accb-pending-requests?department=${department}`)
        .then((response) => {
  ...
  const handleApprove = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/accb-approved-requests", {
  ...
  const handlePause = async () => {
    try {
      const response = await axios.post("http://localhost:3001/api/accb-paused-requests", {
        requestIds: selectedRequestIds,
      });
  ...
  return (
    <div className="accb-pending-requests">
...
export default AccbPendingRequests;
